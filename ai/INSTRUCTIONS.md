...
This scraper extracts job listings for **TURBOMECANICA SA** (CIF: 3156315) from the company's own careers site and from ANOFM, and imports them to peviitor.ro.

Targets: https://turbomecanica.ro/en/careers/available-jobs/, , https://mediere.anofm.ro
...
```
scraper/config/company.json  + scraper/config/scraper.json (sources, selectors)
    │
    ▼
scraper/index.js
    │
    ▼
querySOLR(CIF) - check existing jobs (note our own URLs)
    │
    ▼
...
- **Turbomecanica — listing**: `https://turbomecanica.ro/en/careers/available-jobs/` — pagina publică de posturi deschise (HTML server-rendered)
- **Turbomecanica — sitemap**: `` — permalink-uri canonice `/joburi/<slug>/`
...
| Setting | Value | Where |
|---------|-------|-------|
| Request timeout | 10000 ms | `scraper/config/scraper.json` — `requestTimeoutMs` |
...
| Page delay | 1000 ms between requests | `scraper/config/scraper.json` — `pageDelayMs` |
